...
const double coeff_delta_min = 150;
const double coeff_a_min = 10;
const double coeff_delta_seq = 2500;
const double coeff_a_seq = 50;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      fg[0] += coeff_delta_min * CppAD::pow(vars[delta_start + i], 2);
      fg[0] += coeff_a_min * CppAD::pow(vars[a_start + i], 2);
    }

    // Minimize the value gap between sequential actuations.
    for (int i = 0; i < N - 2; i++) {
      fg[0] += coeff_delta_seq * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += coeff_a_seq * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //std::cout<<"We've made it this far!B"<<std::endl;
